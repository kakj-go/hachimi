...
export const CONTROL_PROTOCOL_VERSION = 30 as const;
...
export type BrowserFileToken = { browserSessionId: BrowserSessionId, token: string, fileName: string, size: number, sha256: string, expiresAtMs: number, };
...
export type BrowserHostSettings = { preferredProfileKind: BrowserProfileKind, latestPairing: BrowserPairing | null, };

export type BrowserImportedDownload = { browserSessionId: BrowserSessionId, downloadToken: string, destination: string, size: number, sha256: string, };
...
export type ChannelEnvelope = { messageId: ChannelMessageId, route: ChannelRouteKey, sender: string, text: string, metadata: unknown, authenticated: boolean, botGenerated: boolean, receivedAtMs: number, };

export type ChannelMessageId = string;

export type ChannelProviderAccount = { id: string, providerId: string, displayName: string, secretRef: string | null, enabled: boolean, routeAllowlist: ChannelRouteKey[], configRevision: number, };

export type ChannelProviderAccountUpsert = { id: string, providerId: string, displayName: string, credential: string | null, enabled: boolean, routeAllowlist: ChannelRouteKey[], expectedConfigRevision: number | null, };

export type ChannelProviderHealth = { providerId: string, state: ChannelProviderHealthState, diagnostic: string | null, configRevision: number, };

export type ChannelProviderHealthState = "disabled" | "starting" | "healthy" | "needs_attention" | "failed";
...
export type ChannelRouteKey = { channel: string, account: string, peer: string, thread: string, };
...
export type ComputerWindowIdentity = { appId: string, processId: number, windowHandle: string, fingerprint: string, title: string, elevated: boolean, protectedDesktop: boolean, hachimiOwned: boolean, };
...
export type DeliveryAttempt = { id: ChannelDeliveryId, route: ChannelRouteKey, idempotencyKey: string, text: string, status: DeliveryAttemptStatus, attempt: number, nextAttemptAtMs: number | null, errorCode: string | null, };

export type DeliveryAttemptStatus = "pending" | "claimed" | "delivered" | "retry_scheduled" | "failed";
...
export type EnterpriseAttachmentDownloadRequest = { context: MutationContext, platform: EnterprisePlatform, accountId: string, eventId: string, remoteId: string, metadataHash: string, runId: RunId, runGeneration: number, idempotencyKey: string, };

export type EnterpriseAttachmentDownloadResult = { artifactId: ArtifactId, contentHash: string, mimeType: string, byteSize: number, duplicate: boolean, };

export type EnterpriseAttachmentMetadata = { platform: EnterprisePlatform, remoteId: string, fileName: string | null, mimeType: string | null, declaredSizeBytes: number | null, downloadRequired: boolean, };

export type EnterpriseEventReceipt = { platform: EnterprisePlatform, accountId: string, eventId: string, eventType: string, payloadHash: string, status: EnterpriseEventReceiptStatus, resultCode: string, receivedAtMs: number, acknowledgedAtMs: number | null, };

export type EnterpriseEventReceiptStatus = "accepted" | "duplicate" | "acknowledged" | "dead_letter";

export type EnterpriseIngressMode = "encrypted_callback" | "stream" | "long_connection";

export type EnterpriseIntegrationAccount = { id: string, platform: EnterprisePlatform, connectorAccountId: ConnectorAccountId | null, channelAccountId: string | null, tenantIdentityHash: string, ingressMode: EnterpriseIngressMode, eventSourceId: string, state: EnterpriseIntegrationState, diagnostic: string | null, credentialRevision: number, updatedAtMs: number, };

export type EnterpriseIntegrationState = "disabled" | "starting" | "healthy" | "rate_limited" | "revoked" | "needs_attention" | "failed";

export type EnterpriseMention = { kind: EnterpriseMentionKind, targetId: string | null, displayText: string | null, };

export type EnterpriseMentionKind = "user" | "bot" | "all";

export type EnterprisePlatform = "wecom" | "ding_talk" | "feishu";

export type EnterprisePluginIdentity = { pluginId: PluginId, platform: EnterprisePlatform, connectorId: string, channelId: string, };

export type EntryProfile = "workbench" | "pet_conversation" | "desktop_control";

export type EnvironmentActivity = { kind: "browser"; browser_tab_id: BrowserTabId; run_id: RunId; domain: string } | { kind: "plan"; plan_id: PlanId; step_id: PlanStepId; description: string };
...
export type GatewayHealth = { running: boolean, startupRegistered: boolean, channels: string[], pendingIngress: number, pendingDeliveries: number, revision: number, };
...
export type IngressReceipt = { messageId: ChannelMessageId, status: IngressStatus, sessionId: SessionId | null, runId: RunId | null, resultCode: string, };

export type IngressStatus = "accepted" | "duplicate" | "rejected" | "claimed" | "completed" | "needs_attention";
...
export type LocalHostCommandRequest = { kind: "browser_begin_pairing" } | { kind: "browser_get_host_settings" } | { kind: "browser_list_permissions" } | { kind: "browser_list_permission_requests" } | { kind: "browser_set_preferred_profile"; profile_kind: BrowserProfileKind } | { kind: "browser_start"; session_id: SessionId; run_id: RunId; profile_kind: BrowserProfileKind; initial_url: string | null; pairing_id: BrowserPairingId | null } | { kind: "browser_grant_site_permission"; context: MutationContext; session_id: SessionId; run_id: RunId; browser_session_id: BrowserSessionId; expected_revision: number; origin: string; capabilities: BrowserCapability[]; decision: BrowserPermissionDecision; network_kind: BrowserNetworkRuleKind; allow_private_network: boolean; expires_at_ms: number | null } | { kind: "browser_revoke_site_permission"; context: MutationContext; session_id: SessionId; run_id: RunId; browser_session_id: BrowserSessionId; expected_revision: number; origin: string } | { kind: "browser_observe"; browser_session_id: BrowserSessionId; run_id: RunId } | { kind: "browser_act"; run_id: RunId; request: BrowserActionRequest } | { kind: "browser_choose_upload"; browser_session_id: BrowserSessionId; run_id: RunId } | { kind: "browser_import_download"; browser_session_id: BrowserSessionId; run_id: RunId; download_token: string; suggested_file_name: string } | { kind: "browser_take_over"; browser_session_id: BrowserSessionId; run_id: RunId } | { kind: "browser_stop"; browser_session_id: BrowserSessionId; run_id: RunId } | { kind: "computer_set_app_rule"; session_id: SessionId; rule: ComputerAppRule } | { kind: "computer_list_windows" } | { kind: "computer_list_global_app_rules" } | { kind: "computer_set_global_app_rule"; rule: ComputerAppRule } | { kind: "computer_remove_global_app_rule"; app_id: string } | { kind: "computer_observe"; session_id: SessionId; run_id: RunId; window_handle: string } | { kind: "computer_act"; run_id: RunId; request: ComputerActionRequest } | { kind: "computer_take_over"; session_id: SessionId } | { kind: "plugin_choose_and_install_bundle" } | { kind: "plugin_choose_and_install" } | { kind: "plugin_install_builtin_sample_crm" } | { kind: "plugin_install_builtin_enterprise"; platform: EnterprisePlatform } | { kind: "plugin_list" } | { kind: "plugin_list_contributions"; plugin_id: PluginId | null } | { kind: "plugin_get_contribution_surface"; plugin_id: PluginId; contribution_id: string } | { kind: "plugin_get"; plugin_id: PluginId } | { kind: "plugin_health_check"; plugin_id: PluginId } | { kind: "plugin_permission_diff"; plugin_id: PluginId } | { kind: "plugin_revision_head"; plugin_id: PluginId } | { kind: "plugin_list_revisions"; plugin_id: PluginId } | { kind: "plugin_lifecycle_journal"; plugin_id: PluginId | null } | { kind: "plugin_set_enabled"; plugin_id: PluginId; enabled: boolean } | { kind: "plugin_rollback"; plugin_id: PluginId; revision: string | null } | { kind: "plugin_uninstall"; plugin_id: PluginId } | { kind: "connector_upsert_account"; account: ConnectorAccountUpsert } | { kind: "connector_list_accounts" } | { kind: "connector_get_account"; account_id: ConnectorAccountId } | { kind: "connector_get_driver_descriptor"; plugin_id: PluginId; connector_id: string } | { kind: "connector_revoke_account"; account_id: ConnectorAccountId } | { kind: "connector_invoke"; request: ConnectorInvocationRequest } | { kind: "channel_loopback_receive"; bearer_token: string; envelope: ChannelEnvelope } | { kind: "channel_mock_poll_push"; envelope: ChannelEnvelope } | { kind: "channel_mock_poll_set_connected"; connected: boolean } | { kind: "channel_mock_poll_drain" } | { kind: "channel_enqueue_delivery"; route: ChannelRouteKey; idempotency_key: string; text: string } | { kind: "gateway_health" } | { kind: "gateway_provider_manifests" } | { kind: "gateway_provider_health" } | { kind: "gateway_list_provider_accounts" } | { kind: "gateway_upsert_provider_account"; account: ChannelProviderAccountUpsert } | { kind: "gateway_set_startup_enabled"; enabled: boolean } | { kind: "gateway_reconcile" };

export type LocalHostCommandResponse = { kind: "cancelled" } | { kind: "browser_pairing"; value: BrowserPairing } | { kind: "browser_host_settings"; value: BrowserHostSettings } | { kind: "browser_session"; value: BrowserSession } | { kind: "browser_permission"; value: BrowserSitePermission } | { kind: "browser_permissions"; value: BrowserPermissionLedgerEntry[] } | { kind: "browser_permission_requests"; value: BrowserPermissionRequest[] } | { kind: "browser_observation"; value: BrowserObservation } | { kind: "browser_action"; value: BrowserActionResult } | { kind: "browser_file_token"; value: BrowserFileToken } | { kind: "browser_imported_download"; value: BrowserImportedDownload } | { kind: "computer_rule"; value: ComputerAppRule } | { kind: "computer_rules"; value: ComputerAppRule[] } | { kind: "computer_windows"; value: ComputerWindowIdentity[] } | { kind: "computer_frame"; value: ComputerFrame } | { kind: "computer_action"; value: ComputerActionResult } | { kind: "computer_taken_over"; value: number } | { kind: "plugin"; value: InstalledPlugin | null } | { kind: "plugins"; value: InstalledPlugin[] } | { kind: "plugin_contributions"; value: InstalledContribution[] } | { kind: "plugin_contribution_surface"; value: PluginContributionSurface } | { kind: "plugin_permission_diff"; value: PluginPermissionDiff | null } | { kind: "plugin_revision_head"; value: PluginRevisionHead | null } | { kind: "plugin_revisions"; value: PluginRevisionRecord[] } | { kind: "plugin_lifecycle_journal"; value: PluginLifecycleJournalRecord[] } | { kind: "removed"; value: boolean } | { kind: "connector_account"; value: ConnectorAccount | null } | { kind: "connector_accounts"; value: ConnectorAccount[] } | { kind: "connector_driver_descriptor"; value: ConnectorDriverDescriptor } | { kind: "connector_invocation"; value: ConnectorInvocationResult } | { kind: "ingress"; value: IngressReceipt } | { kind: "ingresses"; value: IngressReceipt[] } | { kind: "mock_poll_connected"; value: boolean } | { kind: "delivery"; value: DeliveryAttempt } | { kind: "gateway_health"; value: GatewayHealth } | { kind: "channel_provider_manifests"; value: ChannelProviderManifest[] } | { kind: "channel_provider_health"; value: ChannelProviderHealth[] } | { kind: "channel_provider_accounts"; value: ChannelProviderAccount[] } | { kind: "channel_provider_account"; value: ChannelProviderAccount } | { kind: "gateway_reconciled" };
...
export type RuntimeFeatureSet = { runRecovery: boolean, providerExtensions: boolean, providerRemoteContext: boolean, multiAgent: boolean, gitRemoteMutations: boolean, pluginRuntime: boolean, enterpriseIntegrations: boolean, desktopControl: boolean, };
...
export type WorkbenchRoute = "home" | "desktop-control" | "settings/general" | "settings/appearance" | "settings/llm" | "settings/avatar" | "settings/motion" | "settings/voice" | "settings/skills" | "settings/mcp" | "settings/local-hosts" | "developer/motion-lab";
