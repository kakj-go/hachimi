...
export type ChannelGrant = { skillIds: string[], mcpServerIds: string[], connectorSelections: ScheduleConnectorSelection[], readOnlyWorkspaceRoots: string[], networkHosts: string[], };
...
export type PetTurnEvent = { type: "started"; runId: string; session_id: SessionId; agent_run_id: RunId } | { type: "completed"; runId: string; session_id: SessionId; agent_run_id: RunId; text: string; speechQueued: boolean } | { type: "cancelled"; runId: string; session_id: SessionId; agent_run_id: RunId } | { type: "failed"; runId: string; session_id: SessionId; agent_run_id: RunId; code: string; message: string };
...
export type RunOrigin = { kind: "interactive" } | { kind: "handoff"; source_session_id: SessionId; source_run_id: RunId } | { kind: "scheduled"; schedule_id: ScheduleId; task_run_id: TaskRunId; scheduled_for_ms: number; event_context?: ScheduleEventContext | null } | { kind: "channel"; channel: string; account: string; peer: string; thread: string; message_id: ChannelMessageId };
...
export type ScheduleAuthorizationScope = { entryProfile: EntryProfile, workloadOverride: WorkloadKind | null, contextTemplate: ScheduleContextTemplate, toolAllowlist: string[], skillAllowlist: SkillId[], skillRevisions?: ScheduleSkillSelection[], mcpToolAllowlist: McpToolSelection[], permissionConfig: SchedulePermissionConfig, contributionRevisions?: ContributionRevision[], hostGrant?: ScheduleHostGrant, };

export type ScheduleBrowserGrant = { enabled: boolean, documentOrigins: string[], resourceOrigins: string[], capabilities: BrowserCapability[], allowPrivateNetwork: boolean, };

export type ScheduleConnectorSelection = { accountId: ConnectorAccountId, contributionRevision: ContributionRevision, allowedActions: string[], };

export type ScheduleContextTemplate = { kind: "general" } | { kind: "project"; project_id: ProjectId; execution_target: ExecutionTarget } | { kind: "session_continuation"; session_id: SessionId };

export type ScheduleCreateRequest = { context: MutationContext, definition: ScheduleDefinition, authorize: boolean, };

export type ScheduleDefinition = { id: ScheduleId, name: string, enabled: boolean, prompt: string, schedule: ScheduleSpec, entryProfile: EntryProfile, workloadOverride: WorkloadKind | null, contextTemplate: ScheduleContextTemplate, toolAllowlist: string[], skillAllowlist: SkillId[], mcpToolAllowlist: McpToolSelection[], contributionRevisions?: ContributionRevision[], hostGrant?: ScheduleHostGrant, permissionConfig: SchedulePermissionConfig, permissionRevision: number, timeoutMs: number, misfirePolicy: MisfirePolicy, deliveryPolicy: DeliveryPolicy, stopConditions?: ScheduleStopConditions, configRevision: number, createdBy: string, nextRunAtMs: number | null, health: ScheduleHealth, healthReason: string | null, createdAtMs: number, updatedAtMs: number, };
...
export type ScheduleHostGrant = { connectors: ScheduleConnectorSelection[], browser: ScheduleBrowserGrant | null, computerUnattended: boolean, };
...
export type SessionPermissionConfig = { permissionProfile: PermissionProfile, approvalPolicy: ApprovalPolicy, };
...
export type WorkbenchEnvironmentSnapshot = { sessionId: SessionId, checkout: CheckoutRecord, bindingRevision: number, baselineRevision: string | null, changes: EnvironmentChangeSummary, git: EnvironmentGitSummary, handoff: EnvironmentHandoffState, activity: EnvironmentActivity | null, sources: SessionSourceRecord[], revision: number, generatedAtMs: number, };
...
export type WorkbenchTaskStartRequest = { idempotencyKey: string, entryProfile?: EntryProfile, sessionId: SessionId | null, projectId: ProjectId | null, prompt: string, executionTarget: ExecutionTarget | null, behaviorMode: BehaviorMode, approvalPolicy: ApprovalPolicy, attachmentIds: AttachmentId[], /** * Explicit Skill identities selected by the user. This is authoritative * when names collide; `$name` remains only an unambiguous text shortcut. */ skillIds?: SkillId[], };
