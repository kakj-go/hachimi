...
export const CONTROL_PROTOCOL_VERSION = 29 as const;
...
export type BrowserObservation = { id: BrowserObservationId, browserSessionId: BrowserSessionId, runGeneration: number, browserRevision: number, origin: string, title: string, text: string, externalContent: boolean, createdAtMs: number, };
...
export type BrowserSession = { id: BrowserSessionId, profileKind: BrowserProfileKind, ownerSessionId: SessionId, ownerRunId: RunId, runGeneration: number, origin: string | null, taskTabGroup: string, revision: number, status: BrowserSessionStatus, createdAtMs: number, };
...
export type WorkbenchSessionSnapshot = { session: SessionRecord, runs: RunRecord[], events: RunEventEnvelope[], transcript: TranscriptItem[], pendingApprovals: ApprovalRequestRecord[], proposedPlans: ProposedPlan[], artifacts: ArtifactRecord[], agentTasks: AgentTaskRecord[], };
