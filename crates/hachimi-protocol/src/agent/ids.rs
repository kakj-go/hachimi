...
string_id!(SessionSourceId);
string_id!(ApprovalId);
string_id!(TaskRunId);
string_id!(AgentTaskId);
string_id!(AgentTaskMessageId);
string_id!(ForgeOperationId);
string_id!(PlanId);
...
string_id!(RunStepCheckpointId);
string_id!(RunRecoveryId);
string_id!(ProviderEndpointId);
