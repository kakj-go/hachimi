...
Authenticated AppServer request
       ...
       ↓
AgentRunLauncher (事务化 Session/Workspace/Authority snapshot/Run/lineage)
       ↓
...
### 来源、Profile 与 ToolPlan

`RunOrigin::{Project,Manual,Channel,Scheduled,Pet}` 全部进入 `AgentRunLauncher -> AgentRunExecutor -> ToolOrchestrator`。来源只决定审计、权限 owner、队列 lane 和结果投递；handoff 是独立 lineage，不是来源。`EntryProfile`、`WorkloadKind::{General,Coding,Office}` 和 Pet 回复风格只影响模型上下文与展示，不选择 Runtime，也不裁剪工具。

Run 创建事务显式携带权限 owner key：Project/Manual 使用 `session:*`，Channel 使用 `channel_binding:*`，Scheduled 使用 `schedule:*`，Pet 使用 `profile:pet_conversation`。policy、Workspace、不可变 `RunAuthoritySnapshot`、Session/Run 和来源记录在同一事务中提交；Multi-Agent child 使用 transient policy，只继承并收窄父快照，不覆盖来源 owner 的持久配置。

ToolPlan 只按 Provider 能力、功能开关、Host readiness、Plan mode 和权限决策过滤；相同 Host 与权限下，不同来源、EntryProfile 和 workload 得到相同工具集合。交互来源可暴露 `Allow` 与 `RequireApproval` 工具，后台来源只暴露已预授权工具；执行器在 Host dispatch 前始终按结构化参数二次校验。Plan mode 无条件把有效权限收窄为只读。

授权升级与收集用户输入是两个独立维度。`AuthorityMode` 只决定工具越权时能否进入逐次 Approval；`UserInputAvailability` 是当前交互 Host readiness，单独决定是否注册 `request_user_input` 和 MCP elicitation。Pet 使用 `AuthorityMode::Unattended + UserInputAvailability::Available`：它可以在 Pet/Workbench 间继续同一份持久 UserInput，但越出预配置权限仍进入 `NeedsAttention`，不能借提问取得授权。Workbench 可用 UserInput；Channel、后台 Scheduled 和子 Agent 不可用；交互触发的 Scheduled 可用。UserInput 始终只提供数据，不创建 Scope、Grant 或 Approval。

### 统一权限

`AgentPermissionPolicy` 只有只读、可写、完全授权三档，并通过 filesystem、process、network、Browser、Computer、MCP、Connector 七类 `ScopedPermissionRules` 编译为不可变 `RunAuthoritySnapshot` 与内部 `CapabilityGrantSet`。Project/Manual 默认可写并可把精确额外授权持久化到 Session；Channel binding、Schedule definition 和 Pet 全局设置默认只读，只读/可写只能使用预配置范围，越权进入 `NeedsAttention` 而不弹审批。Filesystem 每次调用按规范化实际 target 执行 deny-first root/glob 校验；MCP 比较当前 input schema hash；Connector 比较 account/action/actionHash，只有显式 `readOnlyActions` 才降为只读 effect。完全授权明确使用当前用户的 unrestricted Workspace Host，不套受限 AppContainer，并允许整机文件、进程和网络以及全部已安装、已配置且 Host 就绪的能力；它仍不绕过凭据、配对、Host readiness 或组织强制策略。权限降级或清除额外授权会取消受影响的活跃 Run，旧 snapshot 不得继续执行。
...
- Pet Output Host：Pet 使用同一 Agent 与完整 ToolPlan，独立的只剩展示、回复风格、Motion Catalog 和 TTS 输出；动作不接受任意路径，语音只消费最终稳定 Assistant 文本。存在 secret UserInput 的 Run 强制无声，Approval/UserInput/原始 Tool Result 不作为输出源；取消 Run 同步停止输出，Scheduled Run 默认无动作、无语音。
...
`SessionContextBinding` 仅包含 `Project { project_id, checkout_id } | Workspace { workspace_id }`。Project 只能由用户创建；非 Project 不绑定 Project。Manual、Channel、Pet 为每个顶层 Session 创建持久 managed Workspace；Scheduled 为每个 definition 创建持久 managed Workspace，或使用用户显式选择的普通目录，两种会话模式都复用该目录。显式目录失效时进入 `NeedsAttention`，绝不静默回退。创建失败会补偿删除仅本次 provision 的 managed row/dir；Schedule 删除、managed→selected 切换和启动 reconciliation 只清理严格匹配 `agent-workspaces/<workspace_id>` 的 Hachimi managed 目录，绝不删除 SelectedDirectory。Git/Forge 依赖通用 `WorkspaceHandle`，实际目录是 Git 仓库即可使用，不要求 `project_id` 或交互来源。
...
除完全授权外，Windows runtime attestation 必须同时证明 OS、filesystem、process、network 四项 enforced，restricted token canary、Job Object、ACL、managed Runtime/Git SHA-256 和 deny-all network 均通过后才允许 Workspace 写入、Exec、stdio MCP、Browser/Computer broker 或后台副作用。Restricted launch 同时注册 `PROC_THREAD_ATTRIBUTE_SECURITY_CAPABILITIES` 和 `PROC_THREAD_ATTRIBUTE_HANDLE_LIST`，后者只包含本次启动创建的 child-side stdio handles；无 stdio 时关闭继承。AppContainer Profile、marker 和 managed Runtime 全部位于 per-user data root，首次启动/repair/upgrade 不使用 `runas`、UAC、系统 Git 或系统 Git ACL。Workspace 必须是当前用户所有的本地 NTFS 非保护目录，否则返回稳定迁移错误。Run 保存创建时 Sandbox snapshot，普通权限执行能力只能收窄；完全授权的 unrestricted Host 仍执行 immutable authority、ToolPlan、Host readiness、revision 和 side-effect ledger 校验。
...
Compaction 是唯一上下文裁剪入口，支持 Auto/Manual/ProviderOverflow、本地 checkpoint 和 Responses-only Remote Compaction。正常路径使用 `ModelRuntime::count_tokens` 与 Provider `context_window`/`max_output_tokens` 计算 summary reserve、Claude 风格 tier buffer、预计单轮增长和预测阈值；字符阈值只在 Provider 未报告 context window 时作为 fallback。模型请求先对临时视图执行确定性 Microcompact，永久 transcript 和旧 checkpoint 不变。

只有 checkpoint 完整 Completed 且 `covered_through_sequence` 单调推进才安装新 Model View。压缩单位是完整 `ConversationGroup`，不会拆开 User/Assistant/tool_use/tool_result/continuation；瞬时模型视图可修复 orphan/duplicate/unresolved 工具配对，但 append-only transcript 不改写。最近两个完整回合、当前 Run、Approval/UserInput 受保护。Browser/Computer/image/Office/PDF 大内容在下一次模型请求后只留 bounded Artifact reference，未知 MCP/Connector 工具保守保留 head/tail。

固定 overflow 状态机为 Group 校验 → Microcompact → 重新计数 → Semantic/Remote Compact → 最多删除一个最老完整 Group 后重试 → typed `PromptTooLong`。预测超限触发 Auto/MidRun，Provider `ContextOverflow` 触发 Reactive/MidRun；每个 Run 最多三次上下文重建，Provider overflow 最多重试一次。每次重建刷新 AGENTS、Plan、Skill revision、Session sources、Workspace/Git、MCP/Connector/Host revision 与 Artifact reference，并计入现有 context budget。取消不安装 checkpoint，usage 单调累计；不新增 migration、持久熔断、Session Memory Compact、Partial Compact 或 cache-prefix 优化。

Workbench 只把 trim 后精确等于 `/compact` 的输入识别为 typed manual command；它不创建 Run、不追加 User Item。无 Session、活跃 Run、附件或显式 Skill 选择均稳定拒绝并保留输入；`/compact foo` 仍是普通 Prompt。

## 9. Scheduler 与后台任务

ScheduleDefinition 是持久主列表，Scheduler 只计算 occurrence、claim invocation、创建 fresh TaskRun/Run 并提交统一 launcher。`conversationMode` 默认 `per_run_session`，也可使用 `shared_session`；两种模式都复用 definition Workspace。每次触发重新捕获 generation、StepContext、ToolPlan、权限 snapshot、Plugin/Connector revision 和 Host readiness，不恢复旧 Approval、UserInput secret、临时 Grant、Browser observation、Computer frame、MCP session 或进程 lease。

权限 owner 是 ScheduleDefinition；Prompt/名称/时间/通知变化不改变权限 revision，权限、Context、workload、Skill/MCP/贡献点集合变化会更新 definition revision 并取消受影响的活跃 Run。Browser/Computer 范围只来自统一权限 policy；`HostRevisionSnapshot` 只固定 Connector account/action/content/Host/Schema revision，不承载 Browser 或 Computer 权限。企业附件保留动作 `download_attachment` 必须同时匹配 account、contribution revision 和 action；授权缺失或 revision 漂移映射为 `NeedsAttention`，`connector_invoke` 不能绕过附件专用链。Skill、MCP、Plugin/Connector、Sandbox/Host readiness 漂移同样进入 `NeedsAttention`；后台不等待 Approval/UserInput。
...
实现状态与外部 Gate 使用路线图定义的 `Implemented`、`Local Verified`、`Environment Blocked`、`Not Planned` 四态；本架构文档不维护第二套状态。P1–P8 核心代码与本地单测已完成，尚未运行的 Desktop/真实环境矩阵按路线图逐项披露。这些结果不替代真实 Provider、Chrome/CEF、企业组织或 Windows 身份 Gate，`Environment Blocked` 不计为测试失败。
