...
| Pet / DesktopControl | 不开放                                                     | 不开放                                            | 不开放                                                                         |
...
| RC/GA tag / GitHub Release  | 部分完成           | 不适用         | `publish-release.yml` 已实现；真实五类 Gate 未齐、commit/hash 不一致或存在 skip 时禁止执行                                            |
