...
async function openLocalHosts() {
  ...
  await browser.waitUntil(
    async () => (await $('[data-testid="local-hosts-sandbox-capabilities"]').getText()) === "4/4",
    { timeout: 30_000, timeoutMsg: "Local Host settings did not load enforced Sandbox state" },
  );
}

async function samplePluginRow() {
  await waitForDisplayed('[data-testid="local-hosts-plugin-sample-crm"]');
  return $('[data-testid="local-hosts-plugin-sample-crm"]');
}
...
async function enableGatewayStartup() {
  const toggleSelector = '[data-testid="local-hosts-gateway-startup"]';
    ...
    async () => {
      try {
        const response = await fetch(
          "http://127.0.0.1:42371/v1/channels/loopback-webhook/outbox/claim",
          {
            method: "POST",
...
async function waitForGatewayEndpointClosed() {
  await browser.waitUntil(
    async () => {
      try {
        await fetch("http://127.0.0.1:42371/v1/channels/loopback-webhook/outbox/claim", {
          method: "POST",
          headers: { Authorization: `Bearer ${process.env.HACHIMI_DESKTOP_E2E_LOOPBACK_TOKEN}` },
        });
        return false;
...
async function sendLoopback(messageId) {
  ...
  const response = await fetch("http://127.0.0.1:42371/v1/channels/loopback-webhook", {
    method: "POST",
    headers: {
...
describe("Hachimi local Agent Hosts", () => {
  it("pairs Browser and persists the bundled Plugin/Connector lifecycle", async () => {
    await openLocalHosts();
    await clickWhenReady('[data-testid="local-hosts-browser-pair"]');
    await waitForDisplayed('[data-testid="local-hosts-browser-pairing-code"]');
    const pairingNonce = await $('[data-testid="local-hosts-browser-pairing-code"]').getText();
    expect(pairingNonce.length).toBeGreaterThan(20);
    const extensionId = await pairRealChromeExtension(pairingNonce);
    expect(extensionId).toMatch(/^[a-p]{32}$/);
    await clickWhenReady('[data-testid="local-hosts-refresh"]');
    await expect($(".local-hosts-settings")).toHaveText(expect.stringContaining("confirmed"));
    ...
    await clickWhenReady('[data-testid="local-hosts-open-plugin-ui-dashboard"]');
    await waitForDisplayed('[data-testid="local-hosts-plugin-ui"]');
    const customUiFrame = await $('[data-testid="local-hosts-plugin-ui-frame"]');
    ...
    const disabledFrame = await $('[data-testid="local-hosts-plugin-ui-frame"]');
    await browser.execute(
    ...
    await $('[data-testid="local-hosts-connector-name"]').setValue("Desktop E2E CRM");
    await $('[data-testid="local-hosts-connector-secret"]').setValue("ephemeral-e2e-credential");
    await clickWhenReady('[data-testid="local-hosts-create-sample-account"]');
    await browser.waitUntil(
      async () => {
        for (const row of await $$('[data-testid^="local-hosts-connector-"]')) {
    ...
    await expect($(".local-hosts-settings")).toHaveText(expect.stringContaining("Desktop E2E CRM"));
  });

  it("reconciles a durable mock-poll ingress without a second Agent runtime", async () => {
    await openLocalHosts();
    await clickWhenReady('[data-testid="local-hosts-mock-poll"]');
    await waitForDisplayed('[data-testid="local-hosts-notice"]');
    await expect($('[data-testid="local-hosts-notice"]')).toHaveText(
      expect.stringContaining("mock-poll"),
    );

    await restartApplication();
    await openLocalHosts();
    await clickWhenReady('[data-testid="local-hosts-gateway-reconcile"]');
    await $('[data-testid="local-hosts-notice"]').waitForDisplayed({ timeout: 20_000 });
    await expect($('[data-testid="local-hosts-notice"]')).toHaveText(
  ...
  it("routes authenticated HTTP ingress through one Agent Run and delivers once after restart", async () => {
    ...
    await clickWhenReady('[data-testid="local-hosts-refresh"]');
    await browser.waitUntil(
      async () => /hook_(run|tool)_after_ok/.test(await $(".local-hosts-settings").getText()),
